...
def task13():
    ...
    nums = []
    for i in range(100):
        nums.append(int(num[i*50:(i+1)*50]))
    print(sum(nums))
...
def task18():
    ...
    na = [int(num) for num in numArr if len(num) and int(num)]
    paths = {'0':na[0]}
    currentIndex = 0
    currentLen = 0
    for i in range(0,14):
        firstSubarray = na[currentIndex:(currentIndex + currentLen + 1)]
        secondSubarray = na[(currentIndex + currentLen + 1):(currentIndex + currentLen + 1)+( currentLen + 2)]
        for index, num in enumerate(secondSubarray):
            adjactentFirst = [fi for fi in [index-1, index] if fi>=0 and fi<len(firstSubarray)]
            suitablePaths = [k for k in paths.keys() if int(k[len(k)-1]) in adjactentFirst and len(k) == len(firstSubarray)]
            for k in suitablePaths:
                prevSum = paths[k]
                k += str(index)

                paths.update({k: prevSum + num})

        pathsToDelete = [k for k in paths.keys() if len(k) == i+1]
